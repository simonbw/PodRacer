...
export default class PositionSoundFilter extends Entity {
  onAdd() {
    ...
    this.filter.frequency.value = 2**16;
  ...
  onTick() {
    ...
    this.pan.pan.value = Math.cos(diff.angle) * Util.clamp(distance / PAN_DISTANCE - 0.1);
    this.filter.frequency.value = 2**22 / (distance**1.4 + 0.1);
