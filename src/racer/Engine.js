...
export default class Engine extends Entity {
  constructor([x, y], side, engineDef) {
    ...
    this.engineDef.flaps.forEach(flapDef => {
      ...
      if (
        (this.side === "left" && flapDef.side === "outside") ||
        (this.side === "right" && flapDef.side === "inside")
      ) {
  ...
  setThrottle(value) {
    ...
    if (
      !this.conditions.has("throttle_stuck") &&
      !this.conditions.has("no_thrust")
    ) {
  ...
  setFlaps(left, right) {
    this.flaps.forEach(flap => {
      if (
        flap.direction === ControlFlap.LEFT &&
        !this.conditions.has("left_flap_stuck")
      ) {
        flap.setControl(left);
      }
      if (
        flap.direction === ControlFlap.RIGHT &&
        !this.conditions.has("right_flap_stuck")
      ) {
  ...
  onRender() {
    ...
    this.sprite.healthMeter.lineStyle(
      this.engineDef.healthMeterWidth,
      this.engineDef.healthMeterColor
    );
    this.sprite.healthMeter.moveTo(0, 0.4 * this.size[1]);
    this.sprite.healthMeter.lineTo(
      0,
      (-0.4 * this.size[1] * this.health) / this.engineDef.health
    );
  ...
  doCollisionDamage() {
    ...
    if (damage > 50) {
      ...
      this.conditionTimes[condition] =
        this.conditionTimes[condition] + time || time;
      console.log(
  ...
  boostOn() {
    if (
      !this.conditions.has("boost_stuck") &&
      !this.conditions.has("boost_failure")
    ) {
      this.boosting = true;
    }
  }
