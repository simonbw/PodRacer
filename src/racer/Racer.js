...
export default class Racer extends Entity {
  constructor([x, y], racerDef = RacerDefs.ANAKIN) {
    ...
    this.springs = [];
    // ropes
    [[this.leftEngine, this.pod.leftRopePoint], [this.rightEngine, this.pod.rightRopePoint]]
      .forEach(([engine, podPoint]) => {
        this.springs.push(new RopeSpring(this.pod.body, engine.body, {
    ...
    [[-1, -1], [-1, 1], [1, -1], [1, 1]].forEach(([y1, y2]) => {
      this.springs.push(new LinearSpring(this.leftEngine.body, this.rightEngine.body, {
